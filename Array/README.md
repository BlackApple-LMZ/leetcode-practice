...
### 105 给定树的前序遍历和中序遍历 恢复树 值传递和引用传递导致的时间空间开销必须mark！！！！！！！！！！！！！！ ** top 好题
...
### 240 排序二维数组中的查找 ** top 好题 offer
思路：从右上角向左下角扫描，时间O(m+n);


### 287 找到重复数字 1~n的n+1个数 找到重复数字 ** top 好题 剑指offer
```
Input: [1,3,4,2,2] Output: 2
Input: [3,1,3,4,2] Output: 3
```
思路：用hash，时间O(n)，空间O(n)；用值与下标对应的值交换的方法；用快慢指针，时间O(n)，空间常量，但是只能找到一个重复的数字；用二分查找，这个方法有点妙，1-n的范围，所以每次二分，然后查找数组里对应区间数出现的次数，也是只能找到一个重复的数字，时间O(nlogn)；
...
Input: [1, 2] Output: 2 不存在就返回最大值
```
思路：就像找最大值一样，维护最大3个变量，来新的数就交换比较；和最小堆一样…

### 442 数组重复数字 ** 好题 leetcode第2题
```
Input:[4,3,2,7,8,2,3,1] Output:[2,3]
```
思路：针对重复的情况，自己用了set来排除重复的数，discuss用两次pass来检测重复，第一次只做交换，第二次检测不和index一样的数就是重复的数，这一点很妙！
...
```
思路：自己用了两个指针的方法，但其实用一个数，等于1的时候++，0的时候清0，维护一个最大值就ok；

### 495 求包含组间的最大值？ ** 有点意思
题目：给第一个时刻数组，第二个是持续时间，求最大的覆盖区间长度
```
...
### 695 岛的最大面积 宽搜 ** 好题？
### 697 数组的自由度 * 看看
题目：自由度是说数组出现最多次数的元素的次数；给定一个数组，返回与该数组相同自由度的最小长度；
```
Input: [1, 2, 2, 3, 1] Output: 2
Input: [1,2,2,3,1,4,2] Output: 6
```
思路：就用两个hashmap，一个记录最左，一个记录count，O(n)复杂度；
...
```
思路：dp[i] = min(dp[i-1]+cost[i-1], dp[i-2]+cost[i-2]); 第一次没有做出来！！！！！状态转移方程写错了
